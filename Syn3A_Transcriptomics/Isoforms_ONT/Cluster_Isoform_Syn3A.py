# ============================================================
# Cluster_Isoform_Syn3A.py
#
# Isoform clustering of ONT cDNA reads on JCVI-Syn3A. Adapted from
# the syn1 PacBio FLNC version (Syn1_Transcriptomics/Isoforms_PacBio/Cluster_Isoform.py);
# algorithm and parameters are unchanged. Inputs swapped to the
# syn3A BAM (../ONT_Processing/syn3A.ONT.rep1.sorted.bam) and GFF
# (../Genomes_Input/syn3a_genome.gff3, which includes 3 pseudogene
# records that are now picked up alongside `gene` and `CDS`).
# Outputs land alongside this script in Syn3A_Transcriptomics/Isoforms_ONT/.
#
# -------------------------------------------------------------
# Part 1 — WHY we need clustering (kept for the record)
#
# A "raw isoform" = unique (chrom, strand, pos5p0, pos3p0) tuple
# across primary alignments; pos5p0/pos3p0 are transcript-oriented
# aligned ends (soft-clips excluded; polyA already trimmed upstream
# in ONT_Processing).
#
# Empirical findings from 541,272 primary reads on syn3A ONT
# (rep1):
#
#   * 363,073 unique raw tuples   (88.6% are singletons)
#   * 99th-percentile read-count per tuple = 8; max = 2,613.
#   * Using a bare threshold of n>=10 discards 433,080 reads
#     (80.0% of all reads) — these are NOT noise in aggregate;
#     they live on tuples adjacent to high-count tuples and
#     represent ±wobble around real TSS/TTS positions.
#   * Complete-linkage clustering at eps=10 bp on the n>=10 set
#     collapses 2,777 raw tuples to 493 clusters (5.6x); on the
#     operon-segmentation set (n>=50) it collapses 378 to 46
#     clusters (8.2x). Most of those raw tuples are near-duplicate
#     wobble around a small number of real transcripts.
#
...
#
# Tolerance sweep on this BAM (single-linkage, Chebyshev):
#
...
#   50      37                328
#
# Same bend as on syn1: clusters drop fast 5 -> 10 bp, then start
# to flatten between 10 and 15 bp. 10 bp keeps parity with the
# syn1 PacBio pipeline (same operon-segmentation duplication
# tolerance), returns biologically sensible counts (~46 dominant
# transcription units in the n>=50 set; 757 in the canonical
# n>=50 cluster table), and is corroborated by the cluster-level
# MAD diagnostics, which under complete-linkage with eps=10 are
# capped at 5 bp by construction — ONT 3' ends here actually
# sit well below that cap (median MAD = 0, 99th pct = 5).
#
# -------------------------------------------------------------
...
# In dense highly-expressed operons, a carpet of degradation-
# product 5' ends creates unbroken chains of <=eps-bp links that
# merge multiple real TSSs into one giant cluster (the syn1
# pathological example: a 4,233-bp-wide cluster with 5' MAD = 839
# bp under single-linkage).
#
# Complete-linkage under Chebyshev distance requires that every
# pair of members in a cluster be within eps on both axes —
# equivalently, the cluster's bounding box has diameter <= eps on
# each axis. We enforce this with a bounding-box-augmented union-
# find: a union is accepted only if the merged cluster's bbox
# still fits in an eps-by-eps square. No chaining; O(n log n)
# time, O(n) memory.
#
# -------------------------------------------------------------
...
#      Feature types parsed: `gene`, `CDS`, `pseudogene` — the last
#      is required for syn3A's annotated pseudogenes (0051, 0546,
#      0602), which only appear under that feature type.
...
# ----------------------------
MOTHER_FOLDER = "../.."
BAM_PATH = MOTHER_FOLDER + "/Syn3A_Transcriptomics/ONT/ONT_Processing/syn3A.ONT.rep1.sorted.bam"
GFF_PATH = MOTHER_FOLDER + "/Genomes_Input/syn3a_genome.gff3"

OUT_FOLDER = "."   # write outputs alongside this script (Syn3A_Transcriptomics/Isoforms_ONT/)
...
GENE_FEATURE_TYPES = {"gene", "CDS", "pseudogene"}
...
def load_genes_from_gff(gff_path: str, feature_types=GENE_FEATURE_TYPES) -> pd.DataFrame:
    ...
    # Prefer `gene` records when present; otherwise fall back to CDS. Always
    # keep `pseudogene` records (syn3A annotates some pseudogenes — e.g.
    # JCVISYN3A_0051/0546/0602 — only under this feature type).
    preferred = "gene" if (df["feature_type"] == "gene").any() else "CDS"
    df = df[df["feature_type"].isin({preferred, "pseudogene"})].copy()
