...
This repository holds the complete pipeline, from raw-read retrieval to the analysis outputs.

One could run through all the analysis within one day **Or** directly download the results from Zenodo to check the transcription in Syn1 and Syn3A without running any analysis. See [Browsing transcription at any gene](#browsing-transcription-at-any-gene) for details.
...
flowchart TD
    ...
    TPM["Syn1_Syn3A_Transcriptomics<br/>per-gene TPM + cross-platform QC"]
    TPM --> PROT
...
5. **Proteome** — `Syn1_Syn3A_Proteomics/Proteome_Syn1_Syn3A.ipynb` converts mass-spectrometry iBAQ into absolute protein copies per cell for both organisms, adds syn1 localization (DeepTMHMM + SignalP) and the curated syn3A function hierarchy, and compares the two. It writes `syn1_proteome.tsv` and `syn3A_proteome.tsv` for the downstream stages, plus two interactive pages (see below).
6. **RNA–protein correlation** — `Syn1_Syn3A_Corr_RNA_Proteins/` correlates proteome iPM against sense TPM, one script per organism, and tests how far translation initiation (OSTIR) and codon adaptation explain the residual.
7. **Operons** — `Syn1_Operon/` segments the syn1 PacBio isoforms into 459 operons (`Operon_Segmentation.py`), then characterizes them (`Operon_Annotation.py`): canonical boundaries, UTR lengths, −10 promoter boxes and intrinsic terminators. Terminators come from TransTermHP — `make_transterm_coords.py` builds its gene-coordinate file, and the command that produced `syn1_TransTermHP.txt` is recorded at the top of that file. `build_operon_xlsx.py` collects the per-operon results into one table. Run with `Syn1_Operon/` as the working directory. To look at any single operon or gene, use [`Transcription_Visualization/`](Transcription_Visualization/) (step 10).
8. **RNA processing and non-canonical transcription** — `Syn1_RNase/` (3′ erosion and ribonuclease cleavage-site mapping) and `Syn1_Novel_ORF/` (antisense, intergenic and novel-ORF transcription). Both read the operon tables from step 7.
9. **Genome reduction** — `Genome_Reduction/` runs scripts `01`→`11` in numeric order to recast the syn1→syn3A deletions as operon junctions and quantify the transcriptome/proteome reallocation; `build_reduction_SI.py` then consolidates their tables into one workbook. Run with `Genome_Reduction/` as the working directory.
...
It reads the depth bedGraphs and BAMs from steps 2–3, the proteome tables from step 5, the operon map from step 7, and the deletion map from step 9 — so run those first.

### Or skip the pipeline entirely

Everything above is also published as a standalone archive, so you can browse any gene **without running analysis**. Download it from Zenodo — *Transcription in JCVI-Syn1.0 and Syn3A*, [10.5281/zenodo.22553688](https://doi.org/10.5281/zenodo.22553688), CC-BY-4.0 — unzip, and you have the tool plus every table, BAM and depth track it reads, in the same layout:

```bash
unzip Syn1-Syn3A-Transcription-Visualization.zip
cd Syn1-Syn3A-Transcription-Visualization
conda env create -f environment.yml && conda activate mincell-viz
cd Transcription_Visualization && jupyter lab Transcription_Visualization.ipynb
```

That environment is five Python packages plus `samtools` — none of the pipeline's analysis tooling. The archive is 1.4 GB (about 1.8 GB unpacked), mostly alignments, and carries its own README and file manifest. 
<!-- [`Transcription_Visualization/make_viz_bundle.py`](Transcription_Visualization/make_viz_bundle.py) is what builds it from a working tree. -->
